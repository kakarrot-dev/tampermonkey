...
```text
公开托管的个人 Tampermonkey / Violentmonkey（油猴）脚本仓库。每个脚本服务一个站点或一个明确场景，源码即安装产物。
```

### 主要用户

```text
维护者本人是首要用户；其他用户可以自行审查和安装公开脚本，但仓库不为脚本市场或商业分发优化。
```
...
V2EX/                        第三方 V2EX Polish 参考快照，不视为自有脚本
assets/                      README 使用的仓库 Logo、横幅等长期视觉资产
tasks/
  specs/                     需求规格
  plans/                     开发计划
...
README.md                    仓库说明 + 脚本索引
README.zh-CN.md              README 的简体中文对应版本
```
...
- `V2EX/` 中的第三方文件保留上游作者与许可证信息；没有明确任务时不要修改，不要把其许可证扩展到自有脚本。
- `assets/` 只放 README 或长期文档实际引用的资源；不提交临时截图、候选图或生成中间文件。
...
- `README.md`：英文脚本索引、安装与说明
- `README.zh-CN.md`：与英文 README 事实一致的简体中文版本
...
- `README.md` 为英文主入口，`README.zh-CN.md` 为简体中文版本，顶部互相链接。
- 脚本名称、版本、匹配站点、权限、安装链接和许可证边界在两种语言中必须一致。
- 自有脚本与第三方快照必须明确区分；第三方作者和许可证信息不得省略。
- Logo 和横幅放在 `assets/` 并使用仓库相对路径。视觉更新优先遵守 Claude Cream 配色。
- GitHub badges 只表达可验证事实，不添加虚构的测试、构建、下载量或许可证状态。
...
- 中英文 README 的事实信息必须同步。
- 默认不修改 `V2EX/` 中的第三方参考脚本。
```
